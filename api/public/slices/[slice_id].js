...
function routeSliceId(req) {
    const value = req.query?.slice_id;
...
export default async function handler(req, res) {
    ...
    if (req.method && req.method !== 'GET') return methodNotAllowed(res, 'GET', requestId);

    const store = createSupabaseRestClient();
    const now = () => new Date();
    const { getSlice } = createSliceHandlers({
        store,
        now,
    ...
    try {
        const result = await getSlice({ slice_id: routeSliceId(req) }, req, requestId);

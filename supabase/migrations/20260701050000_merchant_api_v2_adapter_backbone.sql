...
create table public.merchant_orders (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint merchant_orders_org_merchant_external_order_unique unique (org_id, merchant_id, external_order_id),
  constraint merchant_orders_org_merchant_idempotency_key_unique unique (org_id, merchant_id, idempotency_key)
);
...
create table public.merchant_inspections (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint merchant_inspections_org_merchant_job_unique unique (org_id, merchant_id, job_id)
);
...
alter table public.merchant_files
  add constraint merchant_files_org_merchant_file_id_unique unique (org_id, merchant_id, file_id);

alter table public.merchant_slice_jobs
  add constraint merchant_slice_jobs_org_merchant_slice_job_id_unique unique (org_id, merchant_id, slice_job_id);

alter table public.merchant_orders
  add constraint merchant_orders_org_merchant_order_id_unique unique (org_id, merchant_id, order_id);

alter table public.merchant_order_items
  add constraint merchant_order_items_org_merchant_order_item_id_unique unique (org_id, merchant_id, order_item_id);

alter table public.merchant_material_reservations
  add constraint merchant_material_reservations_org_merchant_reservation_id_unique unique (org_id, merchant_id, reservation_id);

alter table public.merchant_batches
  add constraint merchant_batches_org_merchant_batch_id_unique unique (org_id, merchant_id, batch_id);

alter table public.merchant_batch_items
  add constraint merchant_batch_items_org_merchant_batch_item_id_unique unique (org_id, merchant_id, batch_item_id);

alter table public.merchant_job_events
  add constraint merchant_job_events_org_merchant_event_id_unique unique (org_id, merchant_id, event_id);

alter table public.merchant_job_artifacts
  add constraint merchant_job_artifacts_org_merchant_artifact_id_unique unique (org_id, merchant_id, artifact_id);

alter table public.merchant_inspections
  add constraint merchant_inspections_org_merchant_inspection_id_unique unique (org_id, merchant_id, inspection_id);
...
alter table public.merchant_invoices
  add constraint merchant_invoices_org_merchant_invoice_id_unique unique (org_id, merchant_id, invoice_id);

alter table public.merchant_invoice_lines
  add constraint merchant_invoice_lines_org_merchant_invoice_line_id_unique unique (org_id, merchant_id, invoice_line_id);

alter table public.merchant_webhook_endpoints
  add constraint merchant_webhook_endpoints_org_merchant_webhook_id_unique unique (org_id, merchant_id, webhook_id);

alter table public.merchant_webhook_deliveries
  add constraint merchant_webhook_deliveries_org_merchant_delivery_id_unique unique (org_id, merchant_id, delivery_id);

alter table public.merchant_realtime_tokens
  add constraint merchant_realtime_tokens_org_merchant_token_id_unique unique (org_id, merchant_id, token_id);

alter table public.merchant_adapter_events
  add constraint merchant_adapter_events_org_merchant_adapter_event_id_unique unique (org_id, merchant_id, adapter_event_id);

alter table public.merchant_files
  add constraint merchant_files_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_slice_jobs
  add constraint merchant_slice_jobs_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_orders
  add constraint merchant_orders_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_order_items
  add constraint merchant_order_items_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_material_reservations
  add constraint merchant_material_reservations_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_batches
  add constraint merchant_batches_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_batch_items
  add constraint merchant_batch_items_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_job_events
  add constraint merchant_job_events_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_job_artifacts
  add constraint merchant_job_artifacts_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_inspections
  add constraint merchant_inspections_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_post_processing_tasks
  add constraint merchant_post_processing_tasks_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_shipments
  add constraint merchant_shipments_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_shipping_labels
  add constraint merchant_shipping_labels_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_rate_cards
  add constraint merchant_rate_cards_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_invoices
  add constraint merchant_invoices_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_invoice_lines
  add constraint merchant_invoice_lines_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_webhook_endpoints
  add constraint merchant_webhook_endpoints_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_webhook_deliveries
  add constraint merchant_webhook_deliveries_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_realtime_tokens
  add constraint merchant_realtime_tokens_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

alter table public.merchant_adapter_events
  add constraint merchant_adapter_events_merchant_scope_fk
  foreign key (org_id, merchant_id) references public.merchants(org_id, merchant_id)
  on delete cascade;

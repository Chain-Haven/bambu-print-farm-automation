...
function writeLauncher() {
    ...
    for (const [name, content] of sh) {
        ...
        fs.chmodSync(target, 0o755); // executable for local macOS/Pi/Linux testing
    }
    log('wrote Windows (.bat/.ps1) + macOS (.command) + Pi/Linux (.sh) launchers');
}
...
function buildSeaExe() {
    ...
    // On macOS the copied runtime is signed; the signature must be stripped
    // before injection and the result ad-hoc re-signed, or Gatekeeper kills the
    // binary on launch.
